...
object MemChainToAlignBatched {
  ...
  */
  ...
  def calPreResultsOfSW(opt: MemOptType,
    ...
    var srt: Array[SRTType] = new Array[SRTType](chain.seedsRefArray.length)
    ...
    var i = 0
    while (i < chain.seedsRefArray.length) {
    ...
    def initializeCoordinates(coordinates: Array[Array[Int]], chainsFilteredArray: Array[Array[MemChainType]], numOfReads: Int): Boolean = {
      ...
      while (i < numOfReads) {
        ...
        if (chainsFilteredArray(i) != null) {
          coordinates(i)(1) = chainsFilteredArray(i)(0).seedsRefArray.length - 1
          isFinished = false;
        }
        ...
        if (chainsFilteredArray(i) != null) {
          if (coordinates(i)(1) > 0) {
            ...
            if (coordinates(i)(0) == chainsFilteredArray(i).length - 1) {
              ...
              coordinates(i)(0) += 1
              coordinates(i)(1) = chainsFilteredArray(i)(coordinates(i)(0)).seedsRefArray.length - 1
              //isFinished = false;
            }
          }
        }
    ...
    while (!isFinished) {
      ...
      while (i < end) {
        ...
        if (coordinates(i)(1) >= 0) {
          ...
          // if (extensionFlags(i) < regArrays(i).curLength && overlapFlags(i) == chainsFilteredArray(i)(coordinates(i)(0)).seedsRefArray.length) {
  ...
  private def getMaxSpan(opt: MemOptType, pacLen: Long, queryLen: Int, chain: MemChainType): Array[Long] = {
    ...
    val seedMinRBeg = chain.seedsRefArray.map(seed => {
      seed.rBeg - (seed.qBeg + calMaxGap(opt, seed.qBeg))
    }).min
    val seedMaxREnd = chain.seedsRefArray.map(seed => {
      seed.rBeg + seed.len + (queryLen - seed.qBeg - seed.len) + calMaxGap(opt, queryLen - seed.qBeg - seed.len)
    }).max
  ...
  private def checkOverlapping(startIdx: Int, seed: MemSeedType, chain: MemChainType, srt: Array[SRTType]): Int = {
    var breakIdx = chain.seedsRefArray.length
    var i = startIdx
    var isBreak = false

    while (i < chain.seedsRefArray.length && !isBreak) {
